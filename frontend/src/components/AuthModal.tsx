  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 sm:p-8">
        <button
          onClick={onClose}
          className="absolute top-5 right-5 text-slate-400 hover:text-slate-200 p-1 rounded-lg transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <h2 className="text-2xl font-bold text-white tracking-tight">
          {mode === 'login' ? 'Sign in to IntelliRAG' : 'Create an Account'}
        </h2>
        <p className="text-xs text-slate-400 mt-1 mb-6">
          {mode === 'login'
            ? 'Access your authenticated document intelligence environment.'
            : 'Register your workspace identity to secure future documents.'}
        </p>
